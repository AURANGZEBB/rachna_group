    ...
    def ta_get_report_values(self, partner_id=False, date_start=False, date_end=False):
        ...
        if partner_id and partner_id != "None":
            ...
            for line in all_posted_moves_lines:
                if line.journal_id.type in ["sale", "purchase"] and line.account_id.user_type_id.name == ("Receivable" or "Payable"):
                    ...
                    lines_data.append({
                                    ...
                                    'quantity': line.quantity if line.product_id else "",
                                    ...
                                    'gross_amount': "{:,.2f}".format(line.quantity * line.price_unit) if line.quantity and line.price_unit else "",
                                    'discount_percent': "{:,.2f}".format(line.discount) if line.discount else "",
                                    'discount_amount': "{:,.2f}".format(line.discount * line.quantity * line.price_unit / 100) if line.discount else "",
                                    'price_unit': "{:,.2f}".format(line.price_unit) if line.price_unit else "",
                                    'net_amount': "{:,.2f}".format(amount),
            ...
            data = {
                    ...
                    'opening_debit': "{:,.2f}".format(opening_debit_sum),
                    'opening_credit': "{:,.2f}".format(opening_credit_sum),
                    'opening_balance': "{:,.2f}".format(opening_balance),
                    'total_debit': "{:,.2f}".format(total_debit + opening_debit_sum),
                    'total_credit': "{:,.2f}".format(total_credit + opening_credit_sum),
                    'closing_balance': "{:,.2f}".format(net_balance),
